...
pub struct Model {
    ...
    /// Cross-parameter constraint equations. Maps target parameter name to an
    /// arithmetic expression string (same syntax as `Scalar::Expr`). At
    /// evaluation time each equation is resolved in topological order against
    /// `parameters`, and the result replaces/adds the named parameter for the
    /// duration of feature evaluation (without mutating `parameters`).
    ///
