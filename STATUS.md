...
`Tube`, `HollowBox`, `Revolve`, `ExtrudePolygon`, `Sphere`, `Torus`, `Cone`,
`Frustum` primitives, `Translate`/`Rotate`/`Mirror`/`LinearPattern`/
`PolarPattern`/`CornerCut` transforms, and `Union`/`Intersection`/`Difference`
booleans. Every numeric field accepts literals, `$param` references, or
arbitrary arithmetic expressions (`"$plate_x / 2 + sqrt(16)"`).
...
|-------------------------------------------|----------:|---------:|-------:|
| Planar booleans + primitives + validation | 15%       | 90%      | 13.5   |
...
| Manufacturing features (CornerCut)        | 12%       | 5%       | 0.6    |
...
| **Solidworks-tier total**                 | **100%**  |          | **~40.5%** |
| **OpenSCAD-tier (out of 31 SW pts)**      |           |          | **~91%**   |

Started this session at 18% / 57%. Ended at ~40% / 91%.

## What's left to reach 100% Solidworks-tier

The gap is roughly 60 SW pts and is dominated by features that are each
...
- **Manufacturing features — fillet, chamfer, shell, draft** (12 SW pts).
  Each needs offset-surface math the kerf kernel doesn't have yet. Fillet
  alone is ~1 week of focused work.
...
- The kernel handles planar booleans for arbitrary tree depth (the
  chained-DIFF bug fixed this session was a multi-day root-cause hunt).
...
- 394 tests pass, no warnings.

**Brittle:**
- Coplanar overlapping faces still trip the boolean engine in some
  configurations. Not all multi-cylinder unions work.
- Volume measurement returns zero for vase-style revolved solids
  (degenerate seam topology, documented in tests).
