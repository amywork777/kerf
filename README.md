...
- [x] M39a — `sphere_faceted(r, n)` and `capsule_faceted(r, h, n)` all-planar faceted primitives. Both build triangle soups via direct UV parametrisation and import via `from_triangles`; every face is `SurfaceKind::Plane`, so the full planar boolean pipeline applies. 14 unit tests: topology validation, Euler V−E+F=2, volume-vs-analytic convergence, radius-cubed scaling, JSON round-trip, boolean with box.
- [x] M39b — `torus_faceted(r_major, r_minor, n)` all-planar polyhedral torus. Uses the standard toroidal UV grid (n major × n/2 minor segments, CCW outward winding matching `tessellate`). First faceted genus-1 primitive — works directly with booleans. V−E+F=0 verified, genus-1 Euler check, volume convergence, JSON round-trip, boolean chip-diff. Workspace tests: 342 → 348 passing.
