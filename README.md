...
**Validation: 335 tests across 9 suites + sphere_faceted inline tests:**
...
- [x] M38b — `sphere_faceted(r, n)` lat-long planar faceted sphere. All `2*n*(n-1)` faces are planar triangles, so the full boolean pipeline applies without the curved-face limit. Topology: `n*(n-1)+2` vertices, `3*n*(n-1)` edges, `2*n*(n-1)` faces (Euler V-E+F = 2 verified). Volume converges to (4/3)πr³ from below (O(1/n²)); at n=24, ratio ≈ 0.984. Six tests: topology at n=8 and n=16, volume convergence, positive-volume guard, JSON round-trip, radius-scaling law (2r → 8× volume). Also exports `cylinder_faceted` at the `kerf_brep` crate root (previously unexported there). Follow-up fix: `solid_volume` now returns the exact analytic value `(4/3)πr³` for the analytic `sphere(r)` primitive (previously returned 0 because the empty outer loop was taken as "no geometry"). Test suite: 335 → 336.
