...
- [x] M27 — `kerf` CLI binary. `cargo run --bin kerf -- <union|intersection|difference> a.stl b.stl out.stl` runs an end-to-end boolean on any binary STL inputs the importer accepts. Side-overlap difference panics in the stitcher (existing limit, predates the CLI — pre-existing test geometry was corner-cut and nested-cut). Union and intersection ship working. Screenshots: `screenshots/cli_union.png`, `screenshots/cli_intersection.png`.
